...
//! 3. `TRACE_REPLAY_FORMAT_VERSION = 5` (bumped from 4 when ADR-034
//!    landed — adding `Term::FirstAdmit` (twelfth variant) and the
//!    iteration-counter binding for `Term::Recurse`'s step body via
//!    `RECURSE_IDX_NAME_INDEX`. Bumped from 3 when ADR-030 + ADR-033
//!    landed. Older format-4 traces are not forward-compatible because
//!    the Term variant set changed shape);
    ...
    if missing.is_empty() {
        report.push(TestResult::pass(
            ...
            "T6.21 trace byte layout: primitive/certificate discriminants, \
             TRACE_REPLAY_FORMAT_VERSION = 5, and 7 fold_*_digest helpers pinned",
        ));
    } else {
